...
public class HrController {
	...
	public String update(EmployeeVO vo) {
		// 비즈니스로직 : 화면에서 변경입력한 정보로 DB에 변경 저장한 후
	...
	public String modify(int id, Model model) {
		// DB에서 해당 사원정보를 조회해와 수정화면에 출력할수있게 Model객체에 담는다.
	...
	public String info(int id, Model model) {
		// 비즈니스 로직 : DB에서 해당 사원정보를 조회해와서 화면에 출력하도록 model객체에 담기
		model.addAttribute("vo",service.employee_info(id));
