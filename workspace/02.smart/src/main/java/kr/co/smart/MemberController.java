...
public class MemberController {
	...
	public boolean useridCheck(String userid) {
		...
		// DB에 없는 아이디이면 사용가능, 있는 아이디이면 사용불가
	...
	public String logout(HttpSession session, HttpServletRequest request) {
		MemberVO login = (MemberVO)session.getAttribute("loginInfo"); // 로그인 정보를담고
		session.removeAttribute("loginInfo");						 // 로그인정보 삭제처리
		
		String social = login.getSocial();  // NULL값도 있기때문에 따로 뺌
...
//	      -H "Authorization: Bearer AAAAPIuf0L+"
		 ...
		 * "openapi@naver.com", "profile_image":
		 * "https://ssl.pstatic.net/static/pwe/address/nodata_33x33.gif", "age":
		 * "40-49", "gender": "F", "nickname": "", "id": "32742776", "birthday": "10-01"
		 * } }
		 */
