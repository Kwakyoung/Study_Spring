...
$(function(){
	// 버튼 강제 클릭되어져 있게
	$('ul.nav-pills li').eq(0).trigger('click');  // 이벤트 강제발생
})
...
$('ul.nav-pills li').on({
	'click': function(){		// 클릭한 곳에 파란색이 들어오게
		$('ul.nav-pills li a').removeClass('active');
		$(this).children('a').addClass('active');
	...
	'mouseleave': function(){
		$(this).removeClass('shadow')	 // 마우스 땠을때 그림자 사라지게 하기
	}
...
var page = { pageList:10, blockPage:10 };  // 페이지당 보여질 목록수, 블럭당 보여질 페이지수
// 페이지정보 만들기
function makePage( totalList, curPage ){
	$('.pagination').closest('nav').remove(); // 페이지 목록이 있으면 삭제
	
	page.totalList = totalList;	// 총 목록수
	page.curPage = curPage;		// 현재 페이지번호
	page.totalPage = Math.ceil( page.totalList / page.pageList );    // 총 페이지수
	page.totalBlock = Math.ceil( page.totalPage / page.blockPage );  // 총 블럭수
	page.curBlock = Math.ceil( page.curPage / page.blockPage );      // 현재 블럭번호
	page.endPage = page.curBlock * page.blockPage ; 				// 끝 페이지번호
	page.beginPage = page.endPage - (page.blockPage-1) ; 			// 시작페이지 번호
